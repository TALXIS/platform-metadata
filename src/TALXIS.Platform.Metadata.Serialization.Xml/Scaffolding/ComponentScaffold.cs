...
public static class ComponentScaffold
{
    ...
    // Files: sitemap (required). Parameters: app-name (required).
    private static ScaffoldResult ApplyAppModel(ComponentScaffoldRequest request) =>
        AppModelScaffold.Apply(new AppModelScaffoldRequest
        {
            SolutionRootPath = request.SolutionRootPath,
    ...
    // Files: sitemap, area (both required).
    private static ScaffoldResult ApplySiteMapArea(ComponentScaffoldRequest request) =>
    ...
    // Files: app-module (required). Parameters: role-ids (required).
    private static ScaffoldResult ApplyAppSecurityRole(ComponentScaffoldRequest request) =>
        AppSecurityRoleScaffold.Apply(new AppSecurityRoleScaffoldRequest
        {
            SolutionRootPath = request.SolutionRootPath,
            AppModuleFilePath = RequiredFile(request, "app-module"),
    ...
    // Rendered form templates pass "unknown"/"unknownFormId"/"unknownTabId" for parameters the caller left out.
