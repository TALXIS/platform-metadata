...
public sealed class ExportNormalizer
{
    ...
    private static void StripComponentsNotInSource(Workspace exported, Solution exportedSolution, Workspace source, Solution sourceSolution, List<ExportNormalizationChange> changes)
    {
        ...
        foreach (var siteMap in exported.SiteMaps.ToArray())
        {
            if (MatchesRootComponent(sourceSolution, ComponentType.SiteMap, siteMap.UniqueName, id: null)) continue;
            if (source.SiteMaps.Any(s => NameEquals(s.UniqueName, siteMap.UniqueName))) continue;

            exported.RemoveSiteMap(siteMap.UniqueName);
            exportedSolution.RemoveRootComponent(ComponentType.SiteMap, siteMap.UniqueName, id: null);
            RecordStrip(changes, "site map", siteMap.UniqueName, ComponentType.SiteMap, siteMap.UniqueName);
        }
